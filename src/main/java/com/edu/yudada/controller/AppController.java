...
import com.edu.yudada.model.dto.app.AppAddRequest;
import com.edu.yudada.model.dto.app.AppEditRequest;
import com.edu.yudada.model.dto.app.AppQueryRequest;
import com.edu.yudada.model.dto.app.AppUpdateRequest;
...
public class AppController {
    ...
    /**
     * 更新应用（仅管理员可用）
     * 这一类都是直接将最后要变成的状态封装成一个对象，然后后台这边校验是否符合条件
    ...
    /**
     * 分页获取应用列表（仅管理员可用）
     * 根据请求体中的参数，使用querywrapper进行查询过滤
    ...
    public BaseResponse<Page<App>> listAppByPage(@RequestBody AppQueryRequest appQueryRequest) {
        ...
        // 查询数据库 首先看当前是的位置是在第几页，然后查看size 查看每一页能显示多少，最后将这个页的size个对象进行返回
    ...
    public BaseResponse<Page<AppVO>> listMyAppVOByPage(@RequestBody AppQueryRequest appQueryRequest,
        ...
        // 补充查询条件，只查询当前登录用户的数据
        User loginUser = userService.getLoginUser(request);
        appQueryRequest.setUserId(loginUser.getId());// 设置查询条件为当前登录用户的
        long current = appQueryRequest.getCurrent();// 当前页
        long size = appQueryRequest.getPageSize();
        // 限制爬虫
        ThrowUtils.throwIf(size > 20, ErrorCode.PARAMS_ERROR);
        // 查询数据库
        Page<App> appPage = appService.page(new Page<>(current, size),
                appService.getQueryWrapper(appQueryRequest));// 声明一个apprequessst对象之后，使用page进行搜索
        // 获取封装类
        return ResultUtils.success(appService.getAppVOPage(appPage, request));
    }
    ...
    /**
     * 编辑应用（给用户使用）
     * 更新app功能开放给管理员 编辑app功能是开放给用户进行更新
    ...
    /**
     * 应用审核 首先根据应用id查到对应的应用 然后设置应用的审核状态，可以附上审核的信息
     *
     * @param reviewRequest
     * @param request
     * @return
     */
    @PostMapping("/review")  // 审核应用
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> doAppReview(@RequestBody ReviewRequest reviewRequest, HttpServletRequest request) {
        ...
        App oldApp = appService.getById(id);
        ThrowUtils.throwIf(oldApp == null, ErrorCode.NOT_FOUND_ERROR);
        // 如果原先应用的状态已经是一个状态，但是审核的时候 仍然给设置成一个状态，那么这个状态就是无效的
        if (oldApp.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "请勿重复审核");
        }
        // 更新审核状态 获取当前登录的对象 将用户的审核人 设置成为当前的登录的对象
        User loginUser = userService.getLoginUser(request);
        App app = new App();
        app.setId(id);
